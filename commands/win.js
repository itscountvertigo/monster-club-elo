module.exports = {
    ...
    execute(message, args, author, client){
      ...
      message.channel.send(`${p2}, did you lose? Type 'yes' or 'no' in chat.`).then(() => {
      ...
      .then(async message => {
        ...
        if (message.content.toLowerCase() == 'yes' || message.content.toLowerCase() == 'y' || message.content.toLowerCase() == 'yeet') {

          // get info from mongo
          p1Rating = await mongo_funcs.getRating(p1.id, gameMode)
          p2Rating = await mongo_funcs.getRating(p2.id, gameMode)

          console.log(`getRating returned ${p1Rating} for p1 and returned ${p2Rating} for p2.}`)

          p1GP = await mongo_funcs.getGP(p1.id, gameMode)
          p2GP = await mongo_funcs.getGP(p2.id, gameMode)

          console.log(`getGP returned ${p1GP} for p1 and returned ${p2GP} for p2.}`)
          ...
          console.log(`p1's win probility is ${p1WinProb}`)
          console.log(`p2's win probability is ${p2WinProb}`)

          p1NewRating = Math.round(p1Rating + p1Kval * (1 - p1WinProb))
          p2NewRating = Math.round(p2Rating + p2Kval * (0 - p2WinProb))

          console.log(`p1's new rating is ${p1NewRating}`)
          console.log(`p2's new rating is ${p2NewRating}`)
          ...
          .setDescription(`${p1.username} won!`)
          .addFields(
          ...
          console.log(`${p1.username} claimed loss against ${p2.username}, they confirmed. Gamemode: ${gameMode}`)
          } 
